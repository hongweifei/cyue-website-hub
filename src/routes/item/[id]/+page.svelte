<script lang="ts">
	...
	const iconCandidates = $derived(getFaviconCandidates(data.item.url, data.item.icon));
	...
	// 转换为 HTML
	const htmlContent: string = data.markdownContent
		? (marked.parse(data.markdownContent) as string)
		: '<p>暂无详细介绍</p>';
</script>

<svelte:head>
	<title>{data.item.name} - 鸽子导航网</title>
	<meta name="description" content={data.item.info || `${data.item.name} - ${data.item.group}分组`} />
	...
	<article class="item-detail">
		...
		{#if data.markdownContent}
			<div class="item-description">
				...
				<div class="markdown-content">{@html htmlContent}</div>
			</div>
		{/if}
...
<style>
	...
	.item-icon {
		...
		border-radius: 16px;
		overflow: hidden;
		background: linear-gradient(135deg, var(--icon-bg, #f3f4f6) 0%, #e5e7eb 100%);
		display: flex;
		align-items: center;
	...
	.favorite-btn.favorited {
		background: linear-gradient(135deg, var(--accent-color, #ef4444) 0%, #dc2626 100%);
